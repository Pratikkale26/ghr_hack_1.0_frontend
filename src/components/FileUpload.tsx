    ...
    <div className="max-w-2xl mx-auto p-6 bg-gray-900 text-white rounded-xl shadow-lg space-y-6">
      ...
      <div>
        <label className="block text-sm font-medium mb-2 text-white">
          Data File (Images, Videos, ZIP, CSV, JSON) *
        </label>
        <input
          type="file"
          onChange={handleFileChange}
          className="block w-full text-sm file:mr-4 file:py-2 file:px-4 file:rounded-md 
            file:border-0 file:bg-zinc-800 file:text-zinc-400 hover:file:bg-white cursor-pointer"
          accept=".jpg,.jpeg,.png,.gif,.mp4,.mov,.zip,.csv,.json"
        />
        <p className="mt-1 text-xs text-zinc-500">
          Supported formats: JPG, PNG, GIF, MP4, MOV, ZIP, CSV, JSON
        </p>
      </div>
      ...
      <div>
        ...
        <input
          ...
          className="w-full p-3 bg-zinc-800 text-white border border-zinc-700 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none"
          required
        />
      </div>
      ...
      <div>
        ...
        <textarea
          ...
          className="w-full p-3 bg-zinc-800 text-zinc-300 border border-zinc-700 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none"
          rows={3}
          required
        />
      </div>
      ...
      <div className="space-y-4">
        <div>
          <label className="block text-sm font-medium mb-2 text-zinc-400">License Type *</label>
          <select
            className="w-full p-3 bg-zinc-800 text-zinc-300 border border-zinc-700 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none"
            value={licenseType}
        ...
        {licenseType === "CUSTOM" && (
          <div>
            <label className="block text-sm font-medium mb-2 text-zinc-400">Custom Terms *</label>
            <textarea
        ...
        <div>
          ...
          />
          <p className="mt-1 text-xs text-zinc-500">
            License will expire after {validityPeriod} days
          </p>
        </div>
      </div>
